<template id="page-extension_builder_stub">
  <div class="row q-col-gutter-md">
    ...
    << cancel_comment >> -->
      <q-card
        id="<%if preview.is_settings_preview%>previewFocus<%else%>settingsCard<%endif%>"
      >
        <q-card-section
          class="<%if preview.is_preview_mode and not preview.is_settings_preview%>blur-and-disable<%endif%>"
        >
          <div class="row">
            <div class="col">
              <span class="text-h5">extension_builder_stub_name</span>
              <q-btn
                @click="showSettingsDataForm()"
                v-if="<% if settings_data.is_admin_settings_only %>g.user.admin<% else %>true<% endif %>"
                unelevated
                split
                color="primary"
                icon="settings"
    ...
    << cancel_comment >> -->
      ...
      <q-card
        id="<%if preview.is_owner_data_preview%>previewFocus<%else%>ownerDataCard<%endif%>"
        class="q-mt-xs"
      >
        <q-card-section
          class="<%if preview.is_preview_mode and not preview.is_owner_data_preview%>blur-and-disable<%endif%>"
        >
          ...
          >
            ...
            <template v-slot:body="props">
              <q-tr :props="props">
                <q-td auto-width>
                  <% if public_page.has_public_page %>
                  <q-btn
                    flat
                    dense
                    size="xs"
                    ...
                    color="primary"
                    type="a"
                    :href="'/extension_builder_stub/' + props.row.id"
                    target="_blank"
                    class="q-mr-sm"
                    ><q-tooltip>Open public page</q-tooltip></q-btn
                  >
                  <% endif %> <% if owner_data.editable %>
                  <q-btn
                    flat
                    dense
                    size="xs"
      ...
      <q-card
        id="<%if preview.is_client_data_preview%>previewFocus<%else%>clientDataCard<%endif%>"
        class="q-mt-xs"
      >
        <q-card-section
          class="<%if preview.is_preview_mode and not preview.is_client_data_preview%>blur-and-disable<%endif%>"
        >
          <div class="row items-center no-wrap q-mb-md">
            <div class="col">
              <q-input
    ...
    <% if not preview.is_preview_mode %>
        ...
        <q-card-section class="q-pa-none">
          ...
          <q-list>
            ...
            <q-expansion-item group="extras" icon="info" label="More info">
              <q-card>
                <q-card-section>
                  <p>Some more info about extension_builder_stub_name.</p>
                  <small
                    >Created by
                    <a
    ...
    <q-dialog v-model="settingsFormDialog.show" position="top">
      ...
      >
        ...
        <!-- << cancel_comment >>
      << extension_builder_stub_settings_inputs >>
      << cancel_comment >> -->
    ...
    <q-dialog v-model="ownerDataFormDialog.show" position="top">
      ...
      >
        ...
        <!-- << cancel_comment >>
      << extension_builder_stub_owner_inputs >>
      << cancel_comment >> -->
        <div class="row q-mt-lg">
    ...
    <q-dialog v-model="clientDataFormDialog.show" position="top">
      ...
      >
        ...
        <!-- << cancel_comment >>
      << extension_builder_stub_client_inputs >>
      << cancel_comment >> -->
        <div class="row q-mt-lg">
